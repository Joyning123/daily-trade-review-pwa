...
function getNativeAndroidBridge() {
  const bridge = window.TradeDisciplineAndroid;
  if (!bridge || typeof bridge.isNativeApp !== "function") {
    return null;
  }

  return bridge.isNativeApp() ? bridge : null;
}
...
function syncNativeAccountSize(accountSize) {
  const bridge = getNativeAndroidBridge();
  if (!bridge || typeof bridge.updateAccountSize !== "function") {
    return;
  }

  bridge.updateAccountSize(String(Math.max(0, Number(accountSize) || 0)));
}
...
function updateInstallState() {
  ...
  if (isNativeAndroidApp()) {
    dom.installApp.textContent = "安卓 App";
    dom.installApp.disabled = true;
    setInstallStatus("当前正在 APK 中运行");
    return;
  }
...
function recomputeAndPreview() {
  ...
  syncNativeAccountSize(state.accountSize);
...
function downloadMarkdown() {
  ...
  if (bridge && typeof bridge.exportMarkdown === "function") {
    const result = bridge.exportMarkdown(`${state.date}.md`, content) || "已导出";
    const tone = result.startsWith("已") ? "good" : "danger";
    setSaveStatus(result, tone);
    return;
  }
...
async function requestNotificationPermission() {
  ...
  if (bridge && typeof bridge.requestNotificationPermission === "function") {
    const result = bridge.requestNotificationPermission();
    const map = {
      granted: "安卓提醒已开启，每天 09:00 会发通知",
      prompt: "请在系统弹窗中允许通知",
      settings: "系统通知未开启，已打开设置页",
      denied: "系统通知未开启，可点按钮再去设置里打开",
      default: "首次使用请开启系统通知",
      unsupported: "当前安卓环境不支持通知",
    };

    dom.notificationState.textContent = map[result] || map.default;
    window.setTimeout(updateNotificationState, 1600);
    return;
  }
...
function updateNotificationState() {
  ...
  if (bridge && typeof bridge.getNotificationPermissionState === "function") {
    const map = {
      granted: "安卓提醒已开启，每天 09:00 会发通知",
      denied: "系统通知未开启，点按钮去设置里打开",
      default: "首次使用请开启系统通知",
      unsupported: "当前安卓环境不支持通知",
    };

    dom.notificationState.textContent = map[bridge.getNotificationPermissionState()] || map.default;
    return;
  }
...
function init() {
  ...
  window.addEventListener("trade-discipline-android-permission", updateNotificationState);
